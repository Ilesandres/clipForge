...
# -*- coding: utf-8 -*-
"""
Improved URL Clip Processor for ClipForge
Better memory management and error handling for long videos
"""
...
class URLClipProcessorV2:
    """Improved URL clip processor with better memory management"""
    
    def __init__(self, progress_callback: Optional[Callable] = None):
        """Initialize improved URL clip processor"""
    ...
    def process_url_video(self, url: str, output_base_path: Path, 
        ...
        """Process a video from URL with improved memory management"""
        try:
            # Validate URL first
            ...
            self.temp_dir = tempfile.mkdtemp(prefix="clipforge_v2_")
            ...
            print(f"Using improved processor with max segment duration: {self.max_segment_duration}s")
            
            # Calculate clips with improved logic
            clips = self._calculate_clips_improved(video_info['duration'], clip_duration)
            ...
            # Process each clip with improved error handling
            for i, clip_info in enumerate(clips):
                try:
                    ...
                    # Download segment with timeout and retry
                    segment_path = self._download_segment_improved(
    ...
    def _calculate_clips_improved(self, video_duration: float, clip_duration: int) -> List[Dict[str, float]]:
        """Calculate clip segments with improved logic"""
        clips = []
        start_time = 0.0
        ...
        while start_time < video_duration:
            end_time = min(start_time + actual_clip_duration, video_duration)
            clips.append({
                'start': start_time,
                'end': end_time,
    ...
    def _download_segment_improved(self, url: str, start_time: float, duration: float, 
                                 temp_dir: Path, clip_index: int) -> Optional[str]:
        """Download a video segment with improved error handling"""
        try:
            # Create temporary file path
            temp_file = temp_dir / f"clip_{clip_index:03d}.mp4"
            
            print(f"Downloading segment {clip_index + 1}: {start_time:.1f}s - {start_time + duration:.1f}s")
            
            # Download segment using improved method
            ...
            if segment_path and Path(segment_path).exists():
                ...
                print(f"Segment {clip_index + 1} downloaded: {file_size} bytes")
                return segment_path
            else:
                print(f"Failed to download segment {clip_index + 1}")
                return None
            
        except Exception as e:
            print(f"Error downloading segment {clip_index + 1}: {e}")
            import traceback
            traceback.print_exc()
            return None
    ...
    def estimate_processing_time(self, duration: float, clip_duration: int) -> str:
        ...
        # Improved estimation: 20 seconds per clip for download + processing
