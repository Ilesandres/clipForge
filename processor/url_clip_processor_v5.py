...
# -*- coding: utf-8 -*-
"""
URL Clip Processor V5 for ClipForge
Downloads full video once and extracts all segments with FFmpeg audio fix
"""
...
class URLClipProcessorV5:
    """URL clip processor that downloads full video once and handles FFmpeg audio issues"""
    
    def __init__(self, progress_callback: Optional[Callable] = None):
        """Initialize URL clip processor V5"""
    ...
    def process_url_video(self, url: str, output_base_path: Path, 
        ...
        try:
            ...
            self.temp_dir = tempfile.mkdtemp(prefix="clipforge_v5_")
            ...
            print(f"Using V5 processor - downloading full video once and handling FFmpeg audio issues")
            ...
            for i, clip_info in enumerate(clips):
                try:
                    ...
                    # Extract segment from open video clip with FFmpeg audio fix
                    segment_path = self._extract_segment_with_audio_fix(
                    ...
                    time.sleep(0.1)  # Reduced delay for V5
    ...
    def _extract_segment_with_audio_fix(self, start_time: float, duration: float, 
                                      temp_dir: Path, clip_index: int) -> Optional[str]:
        """Extract a segment from the open video clip with FFmpeg audio fix"""
        try:
            ...
            # V5: Handle FFmpeg audio issues with multiple fallback strategies
            success = False
            
            # Strategy 1: Try with audio (default)
            try:
                print(f"Strategy 1: Writing with audio...")
                ...
                # Strategy 2: Try without audio
                try:
                    print(f"Strategy 2: Writing without audio...")
                    segment.write_videofile(
                        str(temp_segment_path),
                        codec='libx264',
                    ...
                    success = True
                    print(f"✅ Strategy 2 successful (no audio)")
                except Exception as e2:
                    print(f"❌ Strategy 2 failed: {e2}")
                    
                    # Strategy 3: Try with different audio codec
                    try:
                        print(f"Strategy 3: Writing with mp3 audio codec...")
                        segment.write_videofile(
                            str(temp_segment_path),
                            codec='libx264',
                            audio_codec='mp3',  # Different audio codec
                            ffmpeg_params=['-preset', 'fast', '-crf', '23'],
                            verbose=False,
                            logger=None
                        )
                        success = True
                        print(f"✅ Strategy 3 successful (mp3 audio)")
                    except Exception as e3:
                        print(f"❌ Strategy 3 failed: {e3}")
                        
                        # Strategy 4: Try with basic settings
                        try:
                            print(f"Strategy 4: Writing with basic settings...")
                            segment.write_videofile(
                                str(temp_segment_path),
                                codec='libx264',
                                audio_codec='aac',
                                ffmpeg_params=['-preset', 'ultrafast'],  # Faster preset
                                verbose=False,
                                logger=None
                            )
                            success = True
                            print(f"✅ Strategy 4 successful (basic settings)")
                        except Exception as e4:
                            print(f"❌ Strategy 4 failed: {e4}")
                            print(f"❌ All strategies failed for clip {clip_index + 1}")
                            return None
    ...
    def estimate_processing_time(self, duration: float, clip_duration: int) -> str:
        ...
        # V5 might take longer due to fallback strategies: 20 seconds per clip
        estimated_seconds = clips_count * 20
